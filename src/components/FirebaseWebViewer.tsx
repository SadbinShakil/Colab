...
interface FirebaseWebViewerProps {
  documentUrl: string
  documentId: string
  userName?: string
  userId?: string
...
export default function FirebaseWebViewer({
  documentUrl,
  documentId,
  userName = 'Anonymous',
  userId = 'anonymous',
      ...
      try {
        ...
        firebaseClient.onAuthStateChanged((user) => {
          if (user) {
            ...
            console.log('👤 Author authenticated:', userName)
            setupWebViewer()
          } else {
  ...
  const setupWebViewer = async () => {
    ...
    try {
      console.log('🔄 Setting up Apryse WebViewer with Firebase collaboration...')
  ...
  const setupFirebaseListeners = (instance: any) => {
    ...
    firebaseClient.onAnnotationCreated((snapshot) => {
      ...
      // The WebViewer will automatically handle the annotation
      // Firebase integration handles the real-time updates
    })
