...
export default function RealSessionReport({
    ...
    const generateReport = async () => {
        ...
        try {
            ...
            if (!sessionData) {
                console.warn('⚠️ No session data found for report generation')
    ...
    if (!report) {
        return (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 backdrop-blur-md p-4">
                <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-4 max-w-md text-center animate-in zoom-in-95 duration-300">
                    <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-2">
                        <AlertCircle className="w-8 h-8 text-slate-400" />
                    </div>
                    <div>
                        <h3 className="text-xl font-bold text-slate-800">No Session Data Available</h3>
                        <p className="text-sm text-slate-500 mt-2">
                            We couldn't generate a report because no active reading session data was found. Try reading for a few minutes first.
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                        className="mt-4 px-6 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 transition-colors"
                    >
                        Close
                    </button>
                </div>
